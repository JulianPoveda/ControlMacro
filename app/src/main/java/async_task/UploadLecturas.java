...
public class UploadLecturas extends AsyncTask<String, Void, Integer> {
    ...
    protected Integer doInBackground(String... params) {
        ...
        for (int j = 0; j < this._tempTabla.size(); j++) {
            ...
            String vinculacion = this.FcnSQL.StrSelectShieldWhere("maestro_clientes", "vinculacion",
                    "nodo='" + nodo + "' AND fecha_programacion ='" + this._tempRegistro.getAsString("fecha_programacion") + "' AND cuenta='" + this._tempRegistro.getAsString("cuenta") + "' AND serie='" + this._tempRegistro.getAsString("serie") + "'");

            this.InformacionCarga += this._tempRegistro.getAsString("fecha_programacion") + "," + this._tempRegistro.getAsString("nodo").replace(" ", "") + "," + this._tempRegistro.getAsString("new_nodo") + "," + this._tempRegistro.getAsString("cuenta") + "," + "" + this._tempRegistro.getAsString("medidor") + "," + this._tempRegistro.getAsString("serie") + "," + this._tempRegistro.getAsString("poste") + "," + "" + this._tempRegistro.getAsString("lectura") + "," + this._tempRegistro.getAsString("observacion").replace(",", "") + "," + this._tempRegistro.getAsString("fecha_registro") + "," + vinculacion.replace("\r\n", "") + "\r\n";
        }
        ...
        for (int j = 0; j < this._tempTabla1.size(); j++) {
            ...
            this.InformacionCargaPoste += "POSTE," + fecha_asignacion + "," + this._tempRegistro1.getAsString("nodo").replace(" ", "") + "," + this._tempRegistro1.getAsString("item") + "," + this._tempRegistro1.getAsString("longitud").replace("°", "-").replace("'", "--").replace("\"", "---") + "," + "" + this._tempRegistro1.getAsString("latitud").replace("°", "-").replace("'", "--").replace("\"", "---") + "," + this._tempRegistro1.getAsString("tipo") + "," + this._tempRegistro1.getAsString("compartido") + "," + "" + this._tempRegistro1.getAsString("estado") + "," + this._tempRegistro1.getAsString("material") + "," + this._tempRegistro1.getAsString("altura") + "," + this._tempRegistro1.getAsString("estructura") + "," + this._tempRegistro1.getAsString("observacion").replace(",", "") + "," + this._tempRegistro1.getAsString("new_nodo").replace("\n", "") + "\r\n";
        }
        ...
        for (int j = 0; j < this._tempTabla1.size(); j++) {
            this._tempRegistro1 = this._tempTabla1.get(j);
            this.InformacionCargaPoste += "EQUIPOS," + fecha_asignacion + "," + this._tempRegistro1.getAsString("id") + "," + this._tempRegistro1.getAsString("nodo").replace(" ", "") + "," + this._tempRegistro1.getAsString("item") + "," + this._tempRegistro1.getAsString("nombre") + "," + "" + this._tempRegistro1.getAsString("capacidad") + "," + this._tempRegistro1.getAsString("unidades") + "\r\n";
        }
